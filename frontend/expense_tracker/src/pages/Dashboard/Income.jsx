...
import { Bar } from "react-chartjs-2"; // make sure imported
...
export default function Income() {
  ...
  useEffect(() => {
    fetch(`${BASE_URL}/api/transactions`, { headers })
      .then((r) => r.json())
      .then((data) => setIncomes(data.filter((t) => t.type === "income")))
      .catch(console.error);
  }, []);
  ...
  const addIncome = async (e) => {
    ...
    const res = await fetch(`${BASE_URL}/api/transactions`, {
      method: "POST",
      headers,
      body: JSON.stringify({ ...form, type: "income" }),
    });
  ...
  const total = incomes.reduce((a, c) => a + c.amount, 0);
  ...
  const donutData = {
    ...
    datasets: [{ data: Object.values(categoryTotals), backgroundColor: ["#4ade80", "#60a5fa", "#facc15", "#a78bfa"] }],
  };

  const lineData = {
    labels: incomes.slice(0, 7).map((t) => new Date(t.date).toLocaleDateString()),
    datasets: [{ label: "Income Trend", data: incomes.slice(0, 7).map((t) => t.amount), borderColor: "#4ade80", tension: 0.4, fill: true }],
  };

  return (
    <div className="space-y-6">
      <div className="bg-white p-4 rounded-2xl shadow flex justify-between items-center">
        <h2 className="text-lg font-semibold text-indigo-700">Total Income</h2>
...
<div className="bg-white p-4 rounded-2xl shadow">
  <h3 className="text-md font-semibold text-gray-700 mb-3">Last 30 Days Income Trend</h3>
  <Bar
    data={{
  ...
  />
      ...
      <form onSubmit={addIncome} className="bg-white p-4 rounded-2xl shadow grid sm:grid-cols-5 gap-3 items-end relative">
      ...
      <div className="bg-white rounded-2xl shadow p-4 overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>{incomes.map((t)=>(<tr key={t._id} className="border-b hover:bg-gray-50"><td>{t.emoji}</td><td>{t.category}</td><td className="text-green-600 font-medium">₹{t.amount}</td><td>{new Date(t.date).toLocaleDateString()}</td><td>{t.note}</td><td><button onClick={()=>del(t._id)} className="text-red-500 hover:text-red-700 text-xs">Delete</button></td></tr>))}</tbody>
        </table>
      </div>
    </div>
  );
